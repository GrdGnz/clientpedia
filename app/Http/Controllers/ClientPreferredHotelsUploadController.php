...
class ClientPreferredHotelsUploadController extends Controller
{
    public function upload(Request $request, $clientId)
    {
        ...
        $fileName = 'preferred_hotels_' . $timestamp . '.' . $file->getClientOriginalExtension();
        $path = $file->storeAs('uploads/preferred_hotels/' . $clientId . '/', $fileName, 'public');

        $upload = new ClientPreferredHotelsUpload();
    ...
    {
        try {
            $upload = ClientPreferredHotelsUpload::findOrFail($id);
