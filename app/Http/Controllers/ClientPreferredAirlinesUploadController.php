...
class ClientPreferredAirlinesUploadController extends Controller
{
    public function upload(Request $request, $clientId)
    {
        ...
        $fileName = 'preferred_airlines_' . $timestamp . '.' . $file->getClientOriginalExtension();
        $path = $file->storeAs('uploads/preferred_airlines/' . $clientId . '/', $fileName, 'public');

        $upload = new ClientPreferredAirlinesUpload();
    ...
    {
        try {
            $upload = ClientPreferredAirlinesUpload::findOrFail($id);
